...
public class StaffModule implements UIConstants {
    ...
    public static void printStaffAssignmentMenu() {
        ...
        for (int i = 0; i < STAFF_ACCOUNTS.length; i++) {
            Staff s = STAFF_ACCOUNTS[i];
              System.out.printf("  %-3d %-10s | %-15s | %-10s%n",
                i + 1,
                s.getStaffId(),
                s.getName(),
                s.getRole()
        ...
        while (loggedIn) {
            ...
            while (!valid) {
                ...
                switch (choice) {
                    ...
                    case "3": valid = true; searchFilterTickets(scanner, staff);         break;
                    ...
                    case "0":
                        ...
                        break;
                    default:
                        System.out.println("  [X] Invalid option. Please select 0 to 6 only.");
                }
            }
        }
    }
    ...
    // 3. SEARCH / FILTER  
    // ---------------------------------------------------------------
private static void searchFilterTickets(Scanner scanner, Staff staff) {
        while (true) {
            System.out.println(W);
            ...
            switch (choice) {
                ...
                case "1": {
                    ...
                    while (filterStatus == null) {
                        ...
                        int idx;
                        try { 
                            idx = Integer.parseInt(sc) - 1; 
                        }catch (NumberFormatException e) { 
                            idx = -1; 
                        }

                        if (idx >= 0 && idx < statuses.length) 
                            filterStatus = statuses[idx];
                        else 
                            System.out.println("  [X] Invalid choice. Please select 1 to 5.");
                    }
                    if (filterStatus == null) 
                        continue; // Go back to main search menu
                ...
                case "2": {
                    ...
                    while (filterCat == null) {
                        ...
                        else System.out.println("  [X] Invalid choice. Please select 1 to 5.");
                    }
