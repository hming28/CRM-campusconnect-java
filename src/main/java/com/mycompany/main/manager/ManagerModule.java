 ...
 */
package com.mycompany.main.manager;
...
import com.mycompany.main.applicant.Applicant;
...
public class ManagerModule implements UIConstants {
    ...
    private static final Manager IDPW_MANAGER = new Manager(
    ...
    private static Manager authenticate(String id, String password) {
        id = id.toUpperCase(); 
        if (IDPW_MANAGER.getManagerId().equals(id) && IDPW_MANAGER.getPassword().equals(password))
            return IDPW_MANAGER;
        return null;
    }
        ...
        while (loggedIn) {
            ...
            while (!valid) {
                ...
                switch (choice) {
                    ...
                    case "5": valid = true; generateMonthlyReport(scanner);    break;
                    ...
                    case "0":
                        ...
                        break;
                    default:
                        System.out.println("  [X] Invalid option. Please select 0 to 7 only.");
                }
            }
        }
    }
        ...
        switch (action) {
            case "1": {
                ...
                Student newStudent = new Student(studentId, selected.getFullName(), tempPass,
                        instEmail, selected.getProgramme(), true, LocalDate.now());
    ...
    private static void generateMonthlyReport(Scanner scanner) {
        YearMonth selected = promptReportMonthYear(scanner);
        if (selected == null) return;
        LocalDate start = selected.atDay(1);
        LocalDate end   = selected.atEndOfMonth();

        List<Ticket> ticketsInMonth = TicketStore.all.stream()
                .filter(t -> t.getCreatedDate() != null
                        && !t.getCreatedDate().isBefore(start)
                        && !t.getCreatedDate().isAfter(end))
                .collect(Collectors.toList());

        int totalTickets  = ticketsInMonth.size();
        int resolved      = (int) ticketsInMonth.stream()
                .filter(t -> "Resolved".equalsIgnoreCase(t.getStatus())
                          || "Closed".equalsIgnoreCase(t.getStatus())).count();
        int pending       = (int) ticketsInMonth.stream()
                .filter(t -> "Pending".equalsIgnoreCase(t.getStatus())).count();
        int inProgress    = (int) ticketsInMonth.stream()
                .filter(t -> "In Progress".equalsIgnoreCase(t.getStatus())).count();
        int reassignments = (int) ticketsInMonth.stream()
                .filter(t -> "Reassignment Requested".equalsIgnoreCase(t.getStatus())).count();

        List<Ticket> timedTickets = ticketsInMonth.stream()
        ...
        List<Applicant> appsInMonth = ApplicantModule.applicantList.stream()
                .filter(a -> a.getSubmissionDate() != null
                        && !a.getSubmissionDate().isBefore(start)
                        && !a.getSubmissionDate().isAfter(end))
                .collect(Collectors.toList());

        int appReceived = appsInMonth.size();
        int appApproved = (int) appsInMonth.stream()
                .filter(a -> "APPROVED".equalsIgnoreCase(a.getStatus())).count();
        int appRejected = (int) appsInMonth.stream()
                .filter(a -> "REJECTED".equalsIgnoreCase(a.getStatus())).count();
        int appPendingC = (int) appsInMonth.stream()
        ...
        List<Ticket> feedbacks = TicketStore.all.stream()
                ...
                .filter(t -> t.getResolvedDate() != null
                        && !t.getResolvedDate().isBefore(start)
                        && !t.getResolvedDate().isAfter(end))
                .collect(Collectors.toList());
        double avgRating = feedbacks.stream().mapToInt(Ticket::getRating).average().orElse(0);

        String month = selected.getMonth().getDisplayName(TextStyle.FULL, Locale.ENGLISH)
                + " " + selected.getYear();

        System.out.println("\n" + W);
        System.out.println("                                               MONTHLY REPORT");
        System.out.println(W);
        System.out.printf("%n  %-24s : %s%n%n", "Period", month);
        ...
        System.out.println(D);
        List<Student> studentsInMonth = StudentStore.studentList.stream()
                .filter(s -> s.getEnrollmentDate() != null
                        && !s.getEnrollmentDate().isBefore(start)
                        && !s.getEnrollmentDate().isAfter(end))
                .collect(Collectors.toList());

        if (studentsInMonth.isEmpty()) {
            System.out.println("  None yet.");
        } else {
            System.out.printf(" | %-10s | %-26s | %-39s | %-20s |%n",
                    "Student ID", "Name", "Course", "Password Status");
            System.out.println(" |" + D.substring(2) + "|");
            for (Student s : studentsInMonth) {
                System.out.printf(" | %s |%n", s.generateSummary());
    ...
    private static YearMonth promptReportMonthYear(Scanner scanner) {
        System.out.println(W);
        System.out.println("                                      SELECT REPORT PERIOD");
        System.out.println(W);
        System.out.println("  Enter 0 at month to cancel.");
        ...
        while (true) {
            ...
            System.out.println("  [X] Invalid month. Please enter 1 to 12.");
        }

        int currentYear = LocalDate.now().getYear();
        int year;
        while (true) {
            System.out.print("  Year  (e.g. " + currentYear + "): ");
            String y = scanner.nextLine().trim();
            try {
                year = Integer.parseInt(y);
                if (year >= 2000 && year <= 2100) break;
            } catch (NumberFormatException ignored) {}
            System.out.println("  [X] Invalid year. Please enter a year between 2000 and 2100.");
        }
