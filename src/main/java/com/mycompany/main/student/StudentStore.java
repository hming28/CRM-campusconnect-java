...
public class StudentStore {
    ...
    // Format per line: id|name|password|email|course|firstLogin|enrollmentDate
    // ---------------------------------------------------------------
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                Student s = new Student(
                    ...
                    null                            // enrollmentDate (legacy rows may not have this)
                );
                if (parts.length >= 7 && !parts[6].trim().isEmpty()) {
                    try {
                        s.setEnrollmentDate(LocalDate.parse(parts[6].trim()));
